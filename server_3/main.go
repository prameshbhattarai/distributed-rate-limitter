...
func routes(e *echo.Echo, cache *groupcache.Group, pool *groupcache.HTTPPool) {
	e.GET("/", func(c echo.Context) error {
		...
		if key == "" {
			return c.String(http.StatusOK, "Server 3 :: Query param not provided")
		}
		...
		if err != nil {
			response := fmt.Sprintf("Server 3 :: Error getting data from cache, %v", err)
		...
		fmt.Printf("Server 3 :: timeWindow %v \n", timeWindow)

		// is limit crossed
		if rateLimit.Count > ALLOWED_LIMIT && timeWindow <= int64(WINDOW) {
			return c.String(http.StatusForbidden, "Server 3 :: Too many request")
		}

		// is timeWindow expired, then reset
		if timeWindow > WINDOW {
			fmt.Printf("Server 3 :: expired %v \n", rateLimit)
			err := cache.Remove(ctx, key)
			if err != nil {
				fmt.Printf("Server 3 :: error while removing cache %v \n", err)
			}
		} else {

			fmt.Printf("Server 3 :: before increment %v \n", rateLimit)

			// increment the counter, and update
			rateLimit.Count += 1
			bs, _ := json.Marshal(rateLimit)

			fmt.Printf("Server 3 :: increment and set %v \n", rateLimit)
			err := cache.Set(ctx, key, bs, time.Now().Add(TIME_TO_LIVE), true)
			if err != nil {
				fmt.Printf("Server 3 :: error while updating cache %v \n", err)
			}
		}
		response := fmt.Sprintf("Server 3 :: Response %v", rateLimit)
...
func main() {
	...
	if err := e.Start(":" + PORT); err != nil {
		log.Fatalf("Error starting Server 3: %v", err)
	}
}
